...
4. Open the **host.cpp** file and observe following sequence of steps carried out by host application:  
    ...
    * **Execution Time Measurement:** Once cpu and FPGA thread are forked, main thread calls **measureExecTimes** function which waits on these threads to finish and also samples the time when these threads finish to measure execution times. 
    * **Ouput Validation and FPGA Peformance:** Once threads finish execution FPGA results are validated by comparing against reference output generated by CPU run and performance comparison is printed out. The performance numbers give metrics such as execution times and throughput(data processing rates).

 9. Now go to **host.cpp** line no. 47 three important parameters are defined here **BATCH_SIZE**, **NUM_OF_BATCHES** and **MAX_SCHEDULED_BATCHES**:
...
This section of the code sets up OpenCL environment for **runFPGA** thread. This kind of setup is very typical of most Vitis acceleration applications and will look very familiar to developers with prior OpenCL experience. The body of code can often be reused as-is from one project to another when accelerating different kernels using Xilnx FPGA cards. Following section provides the briefs of used APIs. Many function calls and object construction call use a macro **OCL_CHECHK** which is used to parse the return status of type **cl_int**. It errors out after OpenCL call, if call doesn't complete as expected.
