...
The previous section explained the SDAccel performance analysis capabilities. The next stage is to utilize these analysis capabilities to drive and measure code optimizations. This lab illustrates the DATAFLOW optimization for the kernel and software pipelining for the host application.
...
1. Confirm that FPGA binary (.awsxclbin file) is indeed present.
	...
	```bash
	# Go the lab folder
	cd /home/centos/SDAccel-AWS-F1-Developer-Labs-master/idct
...
1. Copy the host application executable built in the SDAccel workspace to the local directory and execute the accelerated application on F1 using the FPGA binary.
    ...
    cp /home/centos/SDAccel-AWS-F1-Developer-Labs-master/workspace/IDCT/Emulation-HW/IDCT.exe .
